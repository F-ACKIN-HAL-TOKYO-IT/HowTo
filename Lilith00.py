...
def GET(req):
    ...
    try:
        if req != "/":
            ...
            buf = f.read()
            f.close()
        print("____GET---> " + req + " OK\n")
        return(buf, content_det(req))
    except:
        print("____OPEN FAILED---> " + req + "\n")
...
def handler(socket):
    ...
    if method[0] == "GET":
        ...
        message = make_http_header(server="Lilith/0.0", Content_Length=str(len(content)), Content_Type=content_type) + content
    elif method[0] == "POST":
